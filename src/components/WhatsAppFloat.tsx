...
import { WHATSAPP_TEL_HREF, WHATSAPP_TEL_LABEL, whatsappLink } from "@/lib/constants";

/**
 * Desktop-only Call + WhatsApp FABs. The mobile sticky bottom bar already
 * exposes Call / WhatsApp, so these are hidden below `md`.
 */
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[60] hidden flex-col items-center gap-3 md:flex">
      <a
        ...
        aria-label={`Call ${WHATSAPP_TEL_LABEL}`}
        title={`Call ${WHATSAPP_TEL_LABEL}`}
      >
      ...
      <a
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-emerald-900/30 transition hover:scale-105"
