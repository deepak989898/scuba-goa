/** Trust assurances for the booking page — icon cards matching the redesign mock. */
...
const TRUST_ITEMS = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="#f59e0b" aria-hidden>
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="#22c55e" aria-hidden>
...
export function BookingTrustRow() {
  return (
    <ul
      className="mx-auto mt-5 grid max-w-4xl grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-4"
      aria-label="Booking trust assurances"
    >
      {TRUST_ITEMS.map((item) => (
        <li
          key={item.label}
          className="flex items-center gap-2.5 rounded-2xl border border-white/80 bg-white px-3 py-3 text-sm font-bold text-ocean-900 shadow-md sm:px-3.5"
        >
